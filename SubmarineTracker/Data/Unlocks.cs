...
{
    public record UnlockedFrom(SectorType Sector, bool Sub = false, bool Map = false, bool Main = false)
    {
        public UnlockedFrom(uint sector, bool sub = false, bool map = false, bool main = false) : this((SectorType)sector, sub, map, main) {}
    }
    ...
    public static List<(uint, UnlockedFrom)> FindUnlockPath(uint finalSector)
    {
        ...
        if (final.Sector == SectorType.UnknownUnlock)
            return [];

        var sector = final.Sector;
        var sectorPath = new List<(uint, UnlockedFrom)> { (finalSector, final) };
        while (sector != SectorType.Begin)
        {
            var newSector = SectorToUnlock[(uint)sector];
            sectorPath.Add(((uint)sector, newSector));
